...
function Piece.new(id)
    ...
    this.name = "Sai"
    this.quads = {
        love.graphics.newQuad(2 * TileSize, 0 * TileSize, TileSize, TileSize, PieceImage:getWidth(), PieceImage:getHeight()), -- Player 1
        love.graphics.newQuad(2 * TileSize, 1 * TileSize, TileSize, TileSize, PieceImage:getWidth(), PieceImage:getHeight()), -- Player 2
    }
    this.id = id
    this.x = -1
    this.y = -1
...
function Piece:isDefending(x, y)
    if self.x == -1 or self.y == -1 then return false end -- Not on the board
    if self.x == x and self.y == y then return false end -- We don't cover our own square
    if self.direction == 0 or self.direction == 2 then
        return ( y == self.y and math.abs(self.x - x) == 1 )
    elseif self.direction == 1 or self.direction == 3 then
        return ( x == self.x and math.abs(self.y - y) == 1 )
    end
end
