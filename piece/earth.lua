...
function Piece.new(id)
    ...
    this.name = "Earth"
    this.quads = {
        love.graphics.newQuad(6 * TileSize, 0 * TileSize, TileSize, TileSize, PieceImage:getWidth(), PieceImage:getHeight()), -- Player 1
        love.graphics.newQuad(6 * TileSize, 1 * TileSize, TileSize, TileSize, PieceImage:getWidth(), PieceImage:getHeight()), -- Player 2
    }
    this.id = id
    this.x = -1
    this.y = -1
...
function Piece:isAttacking(x, y)
    ...
    if self.x ~= x and self.y ~= y then return false end -- Different row *and* column
    return math.abs(self.x - x) + math.abs(self.y - y) <= 2
end
...
function Piece:isDefending(x, y)
    return false -- Earth doesn't defend any squares
end
