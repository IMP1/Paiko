...
function Piece.new(id)
    ...
    this.name = "Water"
    this.quads = {
        love.graphics.newQuad(5 * TileSize, 0 * TileSize, TileSize, TileSize, PieceImage:getWidth(), PieceImage:getHeight()), -- Player 1
        love.graphics.newQuad(5 * TileSize, 1 * TileSize, TileSize, TileSize, PieceImage:getWidth(), PieceImage:getHeight()), -- Player 2
    }
    this.id = id
    this.x = -1
    this.y = -1
...
function Piece:isAttacking(x, y)
    ...
    return math.abs(y - self.y) == 1 and math.abs(x - self.x) == 1
end
...
function Piece:isDefending(x, y)
    ...
    if x ~= self.x or y ~= self.y then return false end
    return math.abs(x - self.x) + math.abs(y - self.y) == 1
end
